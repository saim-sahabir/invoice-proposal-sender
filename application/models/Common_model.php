...
class Common_model extends CI_Model {
    ...
    public function getTaxesByCompanyId($company_id)
    {
        $this->db->select("*");
        $this->db->from("tbl_tax_fields");
    ...
    public function getByCompanyId($company_id, $table_name) {
     ...
     function thisGetCompanyInfo() {
        $company_id = $this->session->userdata('company_id');
        $data = $this->db->query("SELECT * FROM tbl_companies 
      WHERE id=$company_id")->row();
        return $data;
    }

    function GetCustomize() {
        $company_id = $this->session->userdata('company_id');
        $data = $this->db->query("SELECT * FROM tbl_customize 
      WHERE id=$company_id")->row();
        return $data;
    }

    function GetCustomizeById($id) {
        $company_id = $id;
        $data = $this->db->query("SELECT * FROM tbl_customize 
      WHERE id=$company_id")->row();
        return $data;
    ...
    public function getByCompanyIdForTax($company_id, $table_name) {
        $result = $this->db->query("SELECT * 
          FROM $table_name 
          WHERE company_id=$company_id AND del_status = 'Live'  
          ORDER BY id")->result();
