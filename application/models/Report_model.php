...
FROM tbl_ingredients i WHERE del_status='Live' AND i.company_id= '$company_id'  ORDER BY i.name ASC")->result();
    ...
    public  function proposalReport($startMonth='',$endMonth='',$status=''){
        if($startMonth || $endMonth || $status):
        $outlet_id = $this->session->userdata('company_id');
        $this->db->select('tbl_proposal_invoice.*,emp.full_name as EmployeedName,tbl_expense_items.name as categoryName');
        ...
        if($status!=''){
            $this->db->where('tbl_proposal_invoice.proposal_status', $status);
        }
        $this->db->where('tbl_proposal_invoice.company_id',$outlet_id);
        $this->db->where('tbl_proposal_invoice.del_status','Live');
        $this->db->order_by('date', 'ASC');
        $query_result = $this->db->get();
        $result = $query_result->result();
        return $result;
